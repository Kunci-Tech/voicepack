...
export function buildPack(profile, opts = {}) {
  ...
  let used = 0;
  // What the pack would cost with nothing dropped. Reported when the budget is
  // too small so the caller can ask for the right number instead of guessing —
  // how much was dropped varies too much to extrapolate from what fitted. On two
  // real profiles here the shortfall was 94 tokens on one and 344 on the other.
  let required = 0;
  const kept = [];
  const dropped = [];
  const included = [];
  ...
  const fullMoves = movesSection(profile.voice, Number.MAX_SAFE_INTEGER);
  if (fullMoves) required += estimateTokens(fullMoves) + 1;
  ...
  if (includeExemplars) {
    ...
    if (ranked.length) {
      ...
      // Every exemplar, whole, plus one heading — the cost of the block if the
      // budget had not been in the way.
      required +=
        headingCost +
        ranked.reduce((n, ex, i) => {
          const label = `-- exemplar ${i + 1}${ex.data?.why ? ` — ${ex.data.why}` : ''} --`;
          return n + estimateTokens(`${label}\n${ex.text}`) + 1;
        }, 0);
  ...
  return {
    ...
    budget: maxTokens,
    // The budget at which `dropped` would be empty. Equal to `tokens` rounded up
    // when nothing was dropped; larger when something was.
