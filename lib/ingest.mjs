 ...
 *
 * The one exception is your own text. An exemplar is a verbatim sample of how
 * we write, so `merge --as exemplar` needs the actual words — and this function
 * used to drop them, which meant the only way to create an exemplar was to
 * re-paste the post by hand at merge time, and the obvious command from the
 * `next:` hint below wrote an empty file. Your own text is kept; a third-party
 * post's is still discarded, because keeping someone else's copy in the data
 * directory is exactly what `provenance: third-party` exists to prevent.
 */
export function synthesize(dataDir, parsed, { profile, channel, own = false, note = null }) {
  ...
  const candidate = {
    ...
    // Only our own words. See the note above.
    text: own ? parsed.text : null,
    instances: 1,
    promotable: false,
...
export function applyCandidate(dataDir, candidate, { as = 'rule', text = null }) {
  ...
  if (as === 'exemplar') {
    ...
    // An exemplar with an empty body is worse than no exemplar at all: it is
    // stamped `provenance: human-written`, so it clears every guard in `lint`,
    // while teaching the agent nothing. Refuse rather than write it.
    const body = (text ?? candidate.text ?? '').trim();
    if (!body) {
      throw new Error(
        `Candidate ${candidate.id} carries no text, so there is nothing to write as an exemplar.\n` +
          `Pass the post verbatim: voicepack merge --id ${candidate.id} --as exemplar --text "..."\n` +
          `Candidates ingested before the text was retained do not carry it; re-ingest the post.`
      );
    }
    ...
    writeText(file, fm + body + '\n');

    // Writing the file is only half the job. `loadExemplars` reads the channel's
    // `exemplars` array, so an unregistered file is invisible: the pack skips it
    // and `lint` goes on warning that the channel has no exemplars. This used to
    // write the file and stop, which made `merge --as exemplar` a silent no-op
    // that reported success.
    const rel = path.posix.join('exemplars', candidate.channel, path.basename(file));
    ...
    ch.exemplars = ch.exemplars ?? [];
    if (!ch.exemplars.includes(rel)) ch.exemplars.push(rel);
    writeJson(chFile, ch);

    return { kind: 'exemplar', file, registered: rel };
  }
