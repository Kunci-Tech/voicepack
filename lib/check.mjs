...
import { requireChannel, channelRules, effectiveBanned, dropSubsumed } from './store.mjs';

const EMOJI_RE = /\p{Extended_Pictographic}/gu;
const HASHTAG_RE = /#[\p{L}\p{N}_]+/gu;

/**
 * Every `detect.type` this file can actually evaluate.
 *
 * Exported because it is a contract, not an implementation detail: a model
 * writing rules will invent plausible type names, and a rule whose detect block
 * is ignored looks identical to a rule that passes. `lint` warns against this
 * list, and the test suite checks it against what the install prompt promises.
 * Add a type here in the same commit as its `case` below.
 */
...
function runDetect(rule, text, st) {
  ...
  // The schemas hand models `"message": ""`, so `d.message ?? fallback` keeps
  // the blank and the violation reads "HARD vp-instagram-001 | " — a report that
  // says nothing at exactly the moment the agent needs to act on it. Prefer the
  // rule text instead.
  const msg = (fallback) => d.message || fallback;
  const fail = (fallback, evidence) => ({ ruleId: id, severity: sev, message: msg(fallback), evidence, source: 'detect' });
  ...
  switch (d.type) {
    case 'regex': {
      ...
      if (m) {
        ...
        return fail(rule.text, `"…${text.slice(idx, m.index + m[0].length + 20).replace(/\n/g, ' ')}…"`);
      }
      return null;
    }
    case 'maxChars':
      return st.chars > d.value ? fail(`Exceeds ${d.value} chars.`, `${st.chars} chars`) : null;
    case 'minChars':
      return st.chars < d.value ? fail(`Under ${d.value} chars.`, `${st.chars} chars`) : null;
    case 'maxEmoji':
      return st.emoji > d.value ? fail(`More than ${d.value} emoji.`, `${st.emoji} emoji`) : null;
    case 'maxHashtags':
      return st.hashtags > d.value ? fail(`More than ${d.value} hashtags.`, `${st.hashtags} hashtags`) : null;
    case 'minHashtags':
      return st.hashtags < d.value ? fail(`Fewer than ${d.value} hashtags.`, `${st.hashtags} hashtags`) : null;
    case 'maxExclamations':
      return st.exclamations > d.value ? fail(`More than ${d.value} exclamation marks.`, `${st.exclamations}`) : null;
    case 'maxSentenceWords':
      return st.maxSentenceWords > d.value
        ? fail(`A sentence runs past ${d.value} words.`, `longest is ${st.maxSentenceWords} words`)
        : null;
    case 'maxParagraphLines':
      return st.maxParagraphLines > d.value
        ? fail(`A paragraph runs past ${d.value} lines.`, `longest is ${st.maxParagraphLines} lines`)
        : null;
    case 'forbiddenWords': {
      // `words` is the documented field. `value` is accepted too, because every
      // other detect type uses `value`, so it is the natural guess — and a rule
      // whose detect block is silently ignored is worse than one that is
      // rejected, since the draft then scores 100 while violating the rule.
      const list = d.words ?? d.value;
      const hits = (Array.isArray(list) ? list : []).filter((w) => new RegExp(escapeRe(w), 'i').test(text));
      return hits.length ? fail(`Uses forbidden wording: ${hits.join(', ')}`, hits.join(', ')) : null;
    }
    default:
      return null;
  }
...
export function checkDraft(profile, { channel, text }) {
  ...
  const banned = effectiveBanned(lex);
  const matched = banned.filter((w) => new RegExp(escapeRe(w), 'i').test(text));
  if (matched.length) {
    // Report the words to delete, not the patterns that fired: overlapping
    // entries in the base list can match the same span, and naming both is noise.
    const hits = dropSubsumed(matched);
    violations.push({
