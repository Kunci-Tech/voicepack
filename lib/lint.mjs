...
import { DETECT_TYPES } from './check.mjs';
...
export function lintProfile(profile) {
  ...
  for (const r of allRules) {
    ...
    // A detect block that cannot run is worse than no detect block at all: the
    // rule looks machine-checked, so a draft that violates it scores 100 and
    // ships. Models invent plausible type names, so this has to be reported.
    if (r.detect && typeof r.detect === 'object') {
      const t = r.detect.type;
      if (!t) {
        push('warn', `Rule "${r.id}" has a detect block with no "type" — it will never run.`);
      } else if (!DETECT_TYPES.includes(t)) {
        push('warn', `Rule "${r.id}" has unknown detect type "${t}" — it will never run. Supported: ${DETECT_TYPES.join(', ')}.`);
      } else if (t === 'forbiddenWords' && !Array.isArray(r.detect.words ?? r.detect.value)) {
        push('warn', `Rule "${r.id}" is forbiddenWords but lists no words — it will never run.`);
      }
    }
  }
